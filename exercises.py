...
def check_letter():
    ...
    alphabet = input('Enter a letter from "ABCDEFGHIJKLMNOPQRSTUVWXYZ": ').lower()
    if alphabet in "AEIOU".lower():
        print(f" the letter {alphabet} is a vowel")
    elif alphabet in "BCDFGHJKLMNPQRSTVWXYZ".lower():
            print(f" the letter {alphabet} is a consonant")
    else: 
        print("Invalid character, please select a letter from A-Z or a-z")
...
def weather_advice():
    # Your control flow logic goes here
    is_cold = input("Is it cold? 'yes' or 'no': ").upper()
    is_raining = input("Is it raining? 'yes' or 'no': ").upper()
...
def guess_number():
    ...
    for guesses in range(5):
        guess = int(input("Type your guess in here: "))
